...
const getTires = async (querys = '') => {
  const origin = window.location.origin,
    ...
    apiSearch = `${tires}${
      querys || '?'
    }&inventario=Rodante&size=20&page=${page}`;
  ...
  try {
    ...
    result.forEach((item) => {
      ...
      if (item.color === 'good') {
        template.querySelector('.tire__status').classList.toggle('good', item.color === 'good');
      } else if (item.color === 'bad') {
        template.querySelector('.tire__status').classList.toggle('bad', item.color === 'bad');
      } else if ( item.color === 'yellow') {
        template.querySelector('.tire__status').classList.toggle('yellow', item.color === 'yellow');
      }
